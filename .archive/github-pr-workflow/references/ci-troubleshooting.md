...
Common patterns when deploying Astro/static sites from GitHub Actions.

#### Pattern A: Deploying repo root instead of built output
```
Signature: Site deploys but new pages 404 (old content still live)
```
**Cause**: The deploy command runs `npx vercel deploy --prod` without building first, or without `--prebuilt`. Vercel's GitHub integration can also mis-detect the output directory if defaulting to the repo root.

**Fix**: Add explicit build + `--prebuilt` to the workflow:
```yaml
...
#### Pattern B: Vercel token not found or expired
```
Signature: Deploy workflow shows 'failure' but within 1 second
```
**Cause**: The `VERCEL_TOKEN` GitHub secret is either missing, expired, or incorrect. Vercel tokens don't expire by default but can be rotated.

**Diagnosis**:
1. Check workflow logs: `gh run view <RUN_ID> --log-failed`
2. If you can't see logs (403), check if the secret exists:
   - Go to GitHub repo → Settings → Secrets and variables → Actions
   - Verify `VERCEL_TOKEN` is present
3. Regenerate the token: Vercel Dashboard → Settings → Tokens → Create

#### Pattern C: Different Vercel project settings
```
Signature: Deploy succeeds but content is stale or missing pages
```
**Cause**: The `.vercel/project.json` settings may have wrong `rootDirectory` or `outputDirectory`. Since `--prebuilt` deploys the local `dist/`, the project settings on Vercel should have rootDirectory and outputDirectory both null.

**Fix**: Verify `.vercel/project.json` has:
```json
{
  "settings": {
    "framework": "astro",
...
#### Pattern D: GitHub Actions integration not wired up
```
Signature: Push to main triggers no deploy at all
```
**Cause**: Vercel may not be auto-connected to the GitHub repo.

**Fix**: Either:
1. Create a `.github/workflows/deploy.yml` that calls Vercel CLI explicitly with `--token '${{ secrets.VERCEL_TOKEN }}'`
2. Or connect via Vercel Dashboard → Import Project → select your GitHub repo

When using option 1, the Vercel project MUST already exist (`vercel link` was run at least once to create `.vercel/project.json`).
...
├── Vercel deploy
│   ├── 404 for new pages → add prebuilt build step
│   ├── Immediate failure → check VERCEL_TOKEN secret
│   └── Stale content → verify project.json settings
