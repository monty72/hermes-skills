...
## Pre/Post-Deployment Security Audit Checklist

For running a security audit before or after a web deployment (run alongside the main hardening workflow):

### Audit Dimensions

**1. IP & Infrastructure Exposure**
Check what the public site reveals about internal network:
```bash
curl -s https://example.com | grep -oE '(192\\.168\\.[0-9]+\\.[0-9]+|10\\.[0-9]+\\.[0-9]+\\.[0-9]+|172\\.(1[6-9]|2[0-9]|3[0-1])\\.[0-9]+\\.[0-9]+)' | sort -u
curl -s https://example.com | grep -oiE '(localhost|:3000|:8081|:8000|:8123|proxmox|hass|powerwall)'
```

**2. Security Headers (Quick Check)**
```bash
...
**4. BREACH Attack Vector Assessment**
- Static sites with no user input reflection → risk is negligible
- Dynamic sites reflecting query params/form input in response → apply mitigations below

**5. Repository Secrets Check**
```bash
...
### 🔴 Critical
[IP exposures, hardcoded secrets, missing auth]

### 🟡 Medium
[Missing headers, CORS wildcard, repo privacy, BREACH risk]
...
**Pre-fix tag:** `git tag pre-security-fix-$(date +%Y%m%d-%H%M%S)` before making changes.

### Workflow
1. Tag pre-fix state
2. Fix dimensions 1→5 in priority order
3. Verify each fix with the audit commands
4. Push to main, Vercel auto-deploys
5. Re-run audit to confirm
6. Document remaining items
