...
- **`unavailable` with `restored: true` means token expired** — When HA entities show `state: "unavailable"` and `attributes.restored: true`, the integration never connected after the last HA restart. This is common for cloud-dependent integrations (Tesla Fleet API, certain weather feeds) whose upstream API tokens expired. **Do NOT assume the device is offline** — the token just needs re-authentication in HA Settings → Devices & Services. For Tesla Powerwall specifically, tokens expire every ~8 hours; check the energy dashboard backend (`~/energy-dashboard/src/energy_api.py`) and the Tesla Owner API fallback (see `netzero-powerwall-api` skill, `references/tesla-owner-api-fallback.md`). **Critical: the HA Tesla integration token and the energy dashboard (`energy_api.py`) token are completely independent.** Refreshing via the Tesla OAuth flow in `api.py` does NOT fix the HA integration's stale token — it must be re-authenticated through HA's UI (Settings → Devices & Services → Tesla Fleet → Re-authenticate).
